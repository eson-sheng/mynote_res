<template>
  <div>
    <group-title> Normal usage, the first one is selected without setting default value {{year1}}</group-title>
    <picker :data='years' v-model='year1' @on-change='change'></picker>
    <br>
    <group-title> Asynchronous loading and dynamic change of data </group-title>
    <picker :data='years001' v-model='year001' @on-change='change'></picker>
    <br>
    <group>
      <cell title="Current value" :value="year001"></cell>
    </group>
    <x-button type="primary" @click.native="changeValue([['1','3','5','7','9','11'],['2','3','4','5'],['a','b','c']])"> Set Data 1 </x-button>
    <x-button type="primary" @click.native="changeValue([['1','3','5','7','9','11'],['2','3','4','5']])">Set Data 2 </x-button>
    <x-button type="primary" @click.native="changeValue([['2','4','6','8','10','11']])">Set Data 3 </x-button>
    <br>
    <group-title> With default value </group-title>
    <picker :data='years' v-model='year2' @on-change='change'></picker>
    <br>
    <group-title> Two-way data binding </group-title>
    ...
    <br>
    <group-title> Multi cols in non chained-mode </group-title>
    ...
    <br>
    <group-title>Locations in chained-mode: Current value {{year7Value}} <br> Text corresponding to the value: {{$refs.picker1&&$refs.picker1.getNameValues()}}</group-title>
    ...
    <br>
    <group-title> 3 cols data but only show 2 cols </group-title>
